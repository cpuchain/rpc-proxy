...
import { JsonRpcReq, JsonRpcResp, BasicRequest, markError, BasicRequestParams } from './request';
...
export function connectWSRpc(request: WebSocketRequest): Promise<WebSocket.WebSocket> {
...
export class WebSocketRequest extends BasicRequest {
    ...
    async connect(): Promise<WebSocket.WebSocket> {
        if (!this.ws) {
