...
import { factory, Logger } from '@cpuchain/logger';
...
import { BlockRespSchema, ErrorObjectSchema, JsonRpcReqSchema, JsonRpcRespSchema } from './schema';
import { JsonRpcReq, markNewError } from './request';
...
export type WebSocketWithRequest = WebSocket.WebSocket & { request: Promise<WebSocketRequest> };
...
export async function initProxy(proxy: Proxy) {
    ...
    if (config.swaggerApi) {
        ...
        let baseDir: string | undefined = path.join(
            process.cwd(),
            './node_modules/@fastify/swagger-ui/static',
        );
        let logo: { type: string; content: Buffer } | undefined = undefined;

        if (existsSync(baseDir)) {
            logo = {
                type: 'image/svg+xml',
                content: await readFile(path.join(baseDir, '/logo.svg')),
            };
        } else {
            baseDir = undefined;
        }
